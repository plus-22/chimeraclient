...
minetest.register_globalstep(function(dtime)
	...
	if item and item:get_count() > 0 and def and def.node_placement_prediction ~= "" then
		...
		if minetest.settings:get_bool("autotnt") then -- little modification here, autoTNT now will only work if the player is holding tnt
            minetest.switch_to_item("mcl_tnt:tnt") 
			local positions = minetest.find_nodes_near_under_air_except(pos, 2, item:get_name(), true)
			for i, p in pairs(positions) do
                minetest.switch_to_item("mcl_tnt:tnt") 
				if i > nodes_per_tick then return end
                    if minetest.switch_to_item("mcl_tnt:tnt") then
